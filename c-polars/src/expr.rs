...
#[no_mangle]
pub unsafe extern "C" fn polars_expr_str_split(
    a: *const polars_expr_t,
    by: *const u8,
    len: usize
) -> *const polars_expr_t {
    let by: &[u8] = std::slice::from_raw_parts(by, len);
    let Ok(by) = std::str::from_utf8(by) else {
        return std::ptr::null();
    };

    let expr = StringNameSpace::split((*a).inner.clone().str(), by);
    make_expr(expr)
}
